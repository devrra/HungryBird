...
class Edrone():
	...
	def __init__(self):
		...
		self.mean_error = self.get_mean_error([self.current_goal[i]-self.drone_position[i] for i in range(3)])
	...
	def get_mean_error(self, input=False):
		mean = 0.0
		if not input:
			for i in range(3):
				mean += (self.error[i].data)**2
		else:
			for i in range(3):
				mean += (input[i])**2		
	...
	#	8. Add error_sum 
		...
		#print "pid :",self.current_goal,"\n",self.mean_error,self.threshold
		while (not rospy.is_shutdown()) and self.mean_error>self.threshold:
			...
			self.mean_error = self.get_mean_error()
			##print self.mean_error
			##print self.mean_error, self.threshold
...
if __name__ == "__main__":
	...
	#if e_drone.get_mean_error(e_drone.init_error)<=e_drone.threshold:
	...
	while e_drone.current_buffer_index < len(e_drone.current_buffer):
		...
		e_drone.mean_error = e_drone.get_mean_error(e_drone.current_goal)
		e_drone.pid()
		#time.sleep(0.5)
		e_drone.current_buffer_index += 1#3	#1 maybe changed if required.
	...
	#if e_drone.get_mean_error(e_drone.goal1_error)<=e_drone.threshold:
	...
	#if e_drone.get_mean_error(e_drone.goal2_error)<=e_drone.threshold:
	...
	e_drone.current_goal = [5.68, -1.91, 33.40, 0.0]
	e_drone.threshold = 0.4
	e_drone.mean_error = e_drone.get_mean_error(e_drone.current_goal)
	e_drone.pid()
